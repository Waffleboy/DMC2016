...
def extraTrees():
    clf = ExtraTreesClassifier(max_depth=8, n_estimators=500,n_jobs=8,random_state=1,
...
def xgboostCV(clf, dataset,target ,useTrainCV=True, cv_folds=5, early_stopping_rounds=25):
    ...
    clf.fit(dataset, target,eval_metric='merror')
...
for all classifiers.
"""
def accuracyChecker(dataset,target,clfs,ensemble,record):
    global accuracy
...
#input: list of TRAINED classifiers, preprocessed test dataset.
def generatePredictions(clfs,test):
    ...
    COM_NAME = socket.gethostname()
    if COM_NAME == 'Waffle':
        test2 = pd.read_csv('E:/Git/DMC2016/thirufiles/orders_class.csv',sep=';')
    else:
        test2 = pd.read_csv('/home/andre/workshop/dmc2016/andrefiles/orders_class.csv',sep=';')
    #make the submission format
    test2 = test2[['orderID','articleID','colorCode','sizeCode']]
    length = len(test2)
    if length != len(test):#catch drop errors
        raise Exception('Preprocessed test and raw test do not have the same number of rows. Check preprocessing.')
    test2['prediction'] = predicted
