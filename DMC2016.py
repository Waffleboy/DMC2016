...
def extraTrees():
    clf = ExtraTreesClassifier(max_depth=8, n_estimators=200,n_jobs=8,random_state=1,
                               class_weight = {1:2})
    return clf
...
for all classifiers. 
"""
def accuracyChecker(dataset,target,clfs,cross_val,ensemble):
    ...
    if ensemble and cross_val == False:
        predictions = np.array(predictions) #transpose it
        predictions = predictions.T
        clf = ExtraTreesClassifier(max_depth = 5,n_jobs=8)
        predicted = cross_validation.cross_val_predict(clf,predictions,testy,cv=5)
        print('5 fold cross val accuracy for ensemble '+str( round(metrics.accuracy_score(testy,predicted),2) ))
        print(metrics.confusion_matrix(testy,predicted,labels=[0,1,2,3,4,5]))
        error = computeError(predicted,testy)
        print('Ensemble Competition metric score : '+str(error))
        print('Ensemble Competition metric score adjusted for train size: '+str(errorScaler(error)))
...
def run():
    ...
    accuracyChecker(dataset,target,clfs,False,True) 
